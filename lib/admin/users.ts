...
export async function fetchUsers(): Promise<User[]> {
    const res = await fetch(`${API_BASE_URL}/api/admin/users/getAllUser`, {
        method : 'GET',
    ...
    return data.map((user: any): User => ({
        ...
        roles: user.roles && Array.isArray(user.roles) ? user.roles : ['customer'], // Sử dụng roles từ Firebase
        phone: user.phone || '',
        gender: user.gender || '',
...
export async function createUser(data: User): Promise<User> {
  ...
  const res = await fetch(`${API_BASE_URL}/api/admin/users/createUser`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  return {
    ...
    roles: responseData.roles && Array.isArray(responseData.roles) ? responseData.roles : ['customer'],
    phone: responseData.phone || '',
    gender: responseData.gender || '',
    dob: responseData.date_of_birth || responseData.dob || '',
    address: responseData.address || '',
    rewardPoints: responseData.reward_points || responseData.rewardPoints || 0,
  };
}
...
export async function updateUser(id: string, data: Partial<User>): Promise<User> {
  ...
  if (data.rewardPoints !== undefined) backendData.reward_points = data.rewardPoints;

  const res = await fetch(`${API_BASE_URL}/api/admin/users/updateUser/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(backendData),
  });
...
export async function deleteUser(id: string): Promise<void> {
  const res = await fetch(`${API_BASE_URL}/api/admin/users/deleteUser/${id}`, {
    method: 'DELETE',
  });
...
export async function setRoleUser(id: string, role: string): Promise<void> {
  const res = await fetch(`${API_BASE_URL}/api/admin/users/setRole/${id}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ role }),
  });
  if (!res.ok) throw new Error('Không thể cập nhật vai trò người dùng');
}