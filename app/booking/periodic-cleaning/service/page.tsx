...
import BookingLayout, { AddressModal } from '../../../../components/booking/BookingLayout';
...
const ServiceStep = () => {
    ...
    const handleExtraToggle = (id: number) => {
        ...
        setBookingData({ ...bookingData, extraServices: newExtras, totalHoursPerSession: bookingData.hoursPerSession + extraSum });
    };
    ...
    return (
        ...
        >
            ...
            {/* Tần suất */}
            <div className="bg-white rounded-xl p-4 mb-4">
                <label className="font-bold text-slate-800 mb-2 block">Tần suất làm việc</label>
                <div className="grid grid-cols-3 gap-2">
                    {pricingData.sessions.map((s) => (
                        <button
                            key={s.id}
                            onClick={() => setBookingData({ ...bookingData, sessionsPerWeek: s.value })}
                            className={`p-2 rounded-lg font-semibold ${bookingData.sessionsPerWeek === s.value ? 'bg-teal-500 text-white' : 'bg-slate-100'}`}
                        >
                            {s.label}
                        </button>
                    ))}
            ...
            {/* Thời lượng */}
            <div className="bg-white rounded-xl p-4 mb-4">
                <label className="font-bold text-slate-800 mb-2 block">Thời lượng mỗi buổi</label>
                <div className="grid grid-cols-3 gap-2">
                    {pricingData.durations.map((d) => (
                        <button
                            key={d.id}
                            onClick={() => setBookingData({ ...bookingData, hoursPerSession: d.value, totalHoursPerSession: d.value })}
                            className={`p-2 rounded-lg font-semibold ${bookingData.hoursPerSession === d.value ? 'bg-teal-500 text-white' : 'bg-slate-100'}`}
                        >
                            {d.label}
                        </button>
                    ))}
            ...
            {/* Dịch vụ thêm */}
            <div className="bg-white rounded-xl p-4 mb-4">
                <label className="font-bold text-slate-800 mb-2 block">Dịch vụ thêm (mỗi buổi)</label>
