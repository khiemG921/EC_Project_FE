...
export const AddressModal = ({ isOpen, onClose, onConfirm }: { isOpen: boolean, onClose: () => void, onConfirm: (address: string) => void }) => {
    ...
    useEffect(() => {
        ...
        const handler = setTimeout(async () => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/location/autocomplete`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    const handleConfirmAddress = async () => {
        ...
        try {
            await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/location/create`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
...
export const TimePickerModal = ({ isOpen, onClose, onSelect }: { isOpen: boolean, onClose: () => void, onSelect: (time: string) => void }) => {
    if (!isOpen) return null;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end justify-center z-50">
            <div className="bg-white rounded-t-2xl shadow-xl w-full max-w-lg">
                <div className="flex justify-between items-center p-4 border-b">
                    <button onClick={onClose} className="text-slate-500 hover:text-slate-800 text-lg">Hủy</button>
                    <h3 className="text-lg font-bold text-slate-800">Chọn giờ bắt đầu</h3>
                    <div className="w-12"></div>
                </div>
                <div className="p-4 max-h-[50vh] overflow-y-auto">
                    <div className="mb-4">
                        <h4 className="font-bold text-slate-600 mb-2">Buổi sáng</h4>
                        <div className="grid grid-cols-4 gap-3">
                            {morningSlots.map(time => <button key={time} onClick={() => onSelect(time)} className="p-2 rounded-lg bg-slate-100 hover:bg-teal-100 text-slate-700 font-semibold">{time}</button>)}
                        </div>
                    </div>
                    <div>
                        <h4 className="font-bold text-slate-600 mb-2">Buổi chiều</h4>
                         <div className="grid grid-cols-4 gap-3">
                            {afternoonSlots.map(time => <button key={time} onClick={() => onSelect(time)} className="p-2 rounded-lg bg-slate-100 hover:bg-teal-100 text-slate-700 font-semibold">{time}</button>)}
                        </div>
                    </div>
                </div>
            </div>
...
export const QuantitySelector = ({ item, quantity, setQuantity }: { item: any, quantity: number, setQuantity: (q: number) => void }) => (
    <div className="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
        ...
        <div className="flex items-center gap-3">
            <button onClick={() => setQuantity(Math.max(0, quantity - 1))} className="w-8 h-8 rounded-full bg-slate-200 text-slate-600 hover:bg-slate-300 transition-colors flex-shrink-0"><i className="fas fa-minus"></i></button>
            <span className="font-bold text-lg w-8 text-center">{quantity}</span>
            <button onClick={() => setQuantity(quantity + 1)} className="w-8 h-8 rounded-full bg-slate-200 text-slate-600 hover:bg-slate-300 transition-colors flex-shrink-0"><i className="fas fa-plus"></i></button>
        </div>
    </div>
);
...
export const ProcessModal = ({ isOpen, onClose, acType }: { isOpen: boolean, onClose: () => void, acType: 'Treo tường' | 'Tủ đứng' | 'Âm trần' }) => {
    ...
    const processContent = {
        ...
        'Tủ đứng': {
            ...
            steps: [ // Giả sử quy trình tương tự máy treo tường
                "**Bước 1: Kiểm tra tình trạng máy:** Bật, kiểm tra và xác nhận tình trạng máy với Khách hàng.",
                "**Bước 2: Vệ sinh bộ lọc và vỏ máy:** Ngắt nguồn điện. Tháo gỡ lưới lọc, mặt nạ. Dùng vòi xịt áp lực cao làm sạch.",
                "**Bước 3: Vệ sinh giàn lạnh:** Che chắn bo mạch. Treo túi hứng nước. Dùng bơm tăng áp xịt rửa giàn lạnh.",
                "**Bước 4: Vệ sinh giàn nóng:** Dùng máy bơm áp lực xịt rửa giàn nóng và khu vực xung quanh.",
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg relative">
                ...
                <div className="p-6 space-y-3 text-slate-600 text-sm max-h-[70vh] overflow-y-auto">
                    {content.note && <p className="font-semibold text-orange-500">{content.note}</p>}
                    {content.steps.map((step, index) => <p key={index} dangerouslySetInnerHTML={{ __html: step.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }}></p>)}
                </div>
            </div>
        </div>
    );
...
export const NotesModal = ({ isOpen, onClose, onConfirm, notes, setNotes }: { isOpen: boolean, onClose: () => void, onConfirm: () => void, notes: string, setNotes: (n:string) => void }) => {
    if (!isOpen) return null;
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
                <h3 className="text-lg font-bold text-slate-800 mb-2">Ghi chú cho Tasker</h3>
                <p className="text-sm text-slate-500 mb-4">Ghi chú này sẽ giúp Tasker làm nhanh và tốt hơn.</p>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Ví dụ: Tầng 1 ngoài ban công có 1 Máy lạnh không mát..." className="w-full p-3 bg-slate-100 border-transparent rounded-lg h-24 resize-none focus:ring-2 focus:ring-teal-500"></textarea>
                <button onClick={onConfirm} className="w-full mt-4 p-3 bg-teal-500 text-white font-bold rounded-lg">Tiếp tục</button>
            </div>
        </div>
    )
};
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-4">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <h3 className="font-bold">{currentMonth.toLocaleString('vi-VN', { month: 'long', year: 'numeric' })}</h3>
                    <button onClick={() => changeMonth(1)}><i className="fas fa-chevron-right"></i></button>
                </div>
                <div className="grid grid-cols-7 gap-1 text-center text-sm mb-2">
                    {['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'].map(day => <div key={day} className="font-semibold text-slate-500">{day}</div>)}
                </div>
                <div className="grid grid-cols-7 gap-1 text-center">{renderCalendar()}</div>
                <button onClick={onClose} className="w-full mt-4 p-3 bg-teal-500 text-white font-bold rounded-lg">Xong</button>
            </div>
        </div>
    );
};
...
const BookingLayout = ({ title, footer, children, onBack }: BookingLayoutProps) => {
    return (
        <div className="bg-slate-100 min-h-screen">
            <header className="bg-white flex items-center p-4 shadow-sm fixed top-0 left-0 right-0 z-30">
                <button onClick={onBack} className="text-xl w-8 text-left text-slate-600 hover:text-slate-800">
                    <i className="fas fa-arrow-left"></i>
                </button>
            ...
            <footer className="bg-white p-4 shadow-top fixed bottom-0 left-0 right-0 z-30">{footer}</footer>
        </div>
    );
};
