...
import {
    Bell, LogOut, User, Heart, History, Settings, LayoutGrid, ChevronDown, Tag, Bookmark
} from 'lucide-react';
                                    ...
                                    <div className="mt-1">
                                        <span className="inline-block px-2 py-1 bg-teal-100 text-teal-700 text-xs rounded-full">
                                            {user.roles?.includes('admin') ? 'Admin' : 
                                             user.roles?.includes('tasker') ? 'Tasker' : 'Khách hàng'}
                                        </span>
                                    </div>
                                </div>
                                <nav className="py-2">
                                    {[...menuItems, ...(user.roles?.includes('tasker') ? [{ label: 'Nhận việc', icon: LayoutGrid, href: '/tasker/find-job' }] : [])].map(item => (
                                        <a 
                                            key={item.label} 
                                            href={item.href} 
